...
| 6 | Output gate closed ⇒ commands ease to neutral within `CUTOFF_HOLD_MS`, then no pulses | `CUTOFF_HOLD_MS` 500 ms ramp, then `detach()` — signal loss, not neutral pulses | `tests/invariants/test_invariant_cutoff_dominates.cpp` (timing + monotone ramp), `tests/invariants/test_invariant_stale_rc_neutral.cpp` |
