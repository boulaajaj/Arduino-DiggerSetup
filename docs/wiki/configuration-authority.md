...
- **Domain and application tunables** — per-domain headers in the
  ...
  thermal thresholds, alert timings, safety windows, Wi-Fi serving
  identity, the shared pin map, and the firmware version single source of
  truth). Values are law: docs describe behavior and never carry a live
  copy.
- **Secrets** — Wi-Fi credentials live in a gitignored `arduino_secrets.h`
  (template committed, real file never); they are not configuration and
  never appear in any doc.
- **Adapter-owned tunables** — deliberately NOT in `src/config/`: a value
  ...
  Wi-Fi serving knobs with the network stack, the S.BUS UART pins with the
  S.BUS adapter). Moving them out would create a second home without a
  second reader.
