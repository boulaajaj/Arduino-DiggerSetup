...
2. **`.claude/hooks/test-gate.py`** — Claude Code `PreToolUse[Bash]` hook
   ...
   gate (23 cases: bypass flags blocked in every spelling — exact, bundled
   short options (`-anm`), abbreviated long options (`--no-verif`),
   glued separators (`true;git commit -n`), wrapper/environment prefixes
   (`FOO=1 git`, `env git`), compound, multi-line, and `git -C` forms;
   inactive `core.hooksPath` blocked in the repo the commit TARGETS; benign
   flags, quoted prose, short-option values, post-`--` pathspecs, and
   heredoc commit messages pass).
