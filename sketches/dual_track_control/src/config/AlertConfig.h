...
// The pattern arrays are C++17 inline (ONE program-wide entity): playback
// and the change-detection compare pattern POINTERS across translation
// units, so per-TU copies would break identity (#189).
#pragma once

#include <stdint.h>

inline const uint16_t BEEP_WIFI_READY[]   = {180, 140, 180};  // Wi-Fi-ready "beep beep": on, off, on (ms)
...
inline const uint16_t ALERT_INACT[]   = {500, 1500};                   // one long beep / 2 s  (RC off)
const int      ALERT_INACT_LEN = 2;
inline const uint16_t ALERT_LOWV[]    = {120, 120, 120, 120, 120, 600};  // three fast chirps / ~1.2 s (low batt)
...
inline const uint16_t ALERT_THERM_WARN[]   = {100, 100};                      // fast nonstop TRILL (>= 80 C)
const int      ALERT_THERM_WARN_LEN = 2;
inline const uint16_t ALERT_THERM_CUT[]    = {500, 150, 500, 150, 500, 1000};  // three LONG beeps (>= 95 C cut)
const int      ALERT_THERM_CUT_LEN  = 6;
inline const uint16_t BEEP_THERM_RESTORED[]   = {120, 100, 120, 100, 120, 100, 120, 1500};  // 4 quick flourish, one-shot (< 75 C)
